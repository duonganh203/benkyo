...
// const useAllDeckCards = (deckIds: string[]) => {
//   const queries = deckIds.map((id) => useGetDeckCards(id));
...
//   const deckIds = deckList.map((d) => d._id!).filter(Boolean);
//   const { isLoading: cardsLoading, data: allCards } = useAllDeckCards(deckIds);

//   const decks = deckList.map((deck, i) => ({
//     ...deck,
//     cards:
//       allCards[i]?.map((c: any) => ({
//         front: c.front || "",
//         back: c.back || "",
//         tags: c.tags || [],
//       })) || [],
...
//   const payload: UpdateMoocPayload = {
//     title,
...
//   if (cardsLoading)
//     return <p className="text-center py-10">Đang tải thẻ (cards) của các deck...</p>;
...
//               <Label htmlFor="isPaid" className="font-semibold">
//                 Paid MOOC
//               </Label>
...
//                 isPaid
//                   ? "max-h-[500px] opacity-100 pt-4 border-t border-border"
//                   : "max-h-0 opacity-0"
//               }`}
//             >
//               {isPaid && (
...
//   {decks.length === 0 ? (
//     <p className="text-gray-400">Không có deck nào.</p>
//   ) : (
//     decks.map((deck, deckIndex) => (
//       <Card key={deck._id || deckIndex} className="p-4 space-y-4">
//         {/* Deck Name */}
//         <div>
...
//         {/* Deck Description */}
//         <div>
//           <Label className="font-medium">Deck description</Label>
//           <Textarea
...
//           {deck.cards.length === 0 ? (
//             <p className="text-gray-400">Không có thẻ nào trong deck này.</p>
//           ) : (
...
//                   placeholder="Front"
//                   value={card.front}
//                   onChange={(e) => updateCard(deckIndex, cardIndex, "front", e.target.value)}
//                 />
//                 <Input
//                   placeholder="Back"
//                   value={card.back}
//                   onChange={(e) => updateCard(deckIndex, cardIndex, "back", e.target.value)}
//                 />
...
export const ClassUpdateMooc = () => {
    ...
    const handleSubmit = () => {
        ...
        const payload = {
            ...
            description: description || undefined,
            decks: decks.map((d) => ({
                deck: d._id ? { _id: d._id } : undefined,
                name: d.name,
                description: d.description || undefined,
                order: d.order,
                cards: d.cards.map((c) => ({
