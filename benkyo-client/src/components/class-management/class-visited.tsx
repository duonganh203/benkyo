...
export const ClassVisited = () => {
    ...
    const {
        ...
        hasNextPage,
        fetchNextPage
    } = useGetClassVisited(classData?._id || '', 10);

    const visitedUsers = pagedVisited?.pages.flatMap((page) => page.data || []) ?? [];
    if (!classData) {
        return (
            <Card>
                <CardHeader>
        ...
        <Card>
            ...
            <CardContent>
                {!visitedUsers || visitedUsers.length === 0 ? (
                    ...
                    <div id='class-visited-scrollable' className='grid gap-4 max-h-[400px] overflow-y-auto pr-1'>
                        <InfiniteScroll
                            dataLength={visitedUsers.length}
                            next={() => fetchNextPage()}
                            hasMore={!!hasNextPage}
                            loader={
                                <div className='flex justify-center py-2 text-xs text-muted-foreground'>
                                    Loading more visited users...
                                </div>
                            }
                            scrollableTarget='class-visited-scrollable'
                                        ...
                                        <div className='flex items-start gap-4'>
                                            <div className='relative'>
                                                <Avatar className='h-12 w-12 ring-2 ring-background'>
                                                    <AvatarImage
                                                        src={visitedUser.user?.avatar}
                                                        alt={visitedUser.user?.name}
                                                    />
                                                    <AvatarFallback className='text-sm font-medium bg-primary/10 text-primary'>
                                                        {visitedUser.user?.name
                                                            ? visitedUser.user.name.charAt(0).toUpperCase()
                                                            : 'U'}
                                                    </AvatarFallback>
                                                </Avatar>
                                                ...
                                                <div className='flex items-center justify-between mb-2'>
                                                    <div>
                                                        <h3 className='font-semibold text-foreground truncate'>
                                                            {visitedUser.user?.name || 'Unknown User'}
                                                        </h3>
                                                        <p className='text-sm text-muted-foreground truncate'>
                                                            {visitedUser.user?.email}
                                                        </p>
                                                    </div>
